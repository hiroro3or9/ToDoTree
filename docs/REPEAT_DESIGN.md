...
既存のカード内に、細いベクターのループ矢印と `2 / 3 回` のバッジを置く。
右下に「1回達成」ボタンを置き、既存の折りたたみ操作と重ならない領域を確保する。
通常・ミニマル双方のカード寸法とレイアウト余白は、実装時に `NodeStyleMetrics` を基準に調整する。

自己ループはカードのバッジ内で短い円弧として表し、キャンバス外側へ大きく張り出す線は描かない。
接続ドラッグ中だけ「繰り返しを設定」のプレビューを表示する。
バッジを押すと設定を開く。矢印やバッジは依存線のヒットテスト対象には含めない。

これにより通常の線・ブロックの枠・上部接続点と重なりにくくする。
線のDelete・通過点追加・線の色変更は自己ループには適用しない。

### ミニマル表示・ブロック折りたたみ

ミニマル表示は名前に添えて小さなループ印と `2/3` を表示する。
加算は選択後のSpace・右クリック・詳細パネルから行う。
完了非表示の対象になるのは最終回の後。途中の達成ではカードを消さない。

畳んだブロックでは個々の回数とループ印を非表示にし、現在の項目単位の完了件数を維持する。
