...
public class ConnectionSideTests
{
    ...
    public async Task ExplicitSide_PreservesAnchorAndOutwardTangent()
    {
        foreach (var size in new[] { new Vec2(24, 24), new Vec2(224, 88) })
            ...
            {
                var (Start, _, Control1, _) = CurveGeometry.BetweenNodes(new Vec2(0, 0), target, size.X, size.Y, ConnectionSide.Top);
                await Assert.That(Start).IsEqualTo(new Vec2(size.X / 2, 0));
                await Assert.That(Control1.X).IsEqualTo(Start.X);
                await Assert.That(Control1.Y < Start.Y).IsTrue();
