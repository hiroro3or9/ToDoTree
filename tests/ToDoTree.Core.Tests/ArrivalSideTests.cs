        ...
        foreach (var size in new[] { new Vec2(24, 24), new Vec2(224, 88) })
            foreach (var fromSide in Enum.GetValues<ConnectionSide>())
                foreach (var to in new[] { new Vec2(320, 310), new Vec2(-320, -310) })
                    ...
                    {
                        var (Start, End, Control1, Control2) = CurveGeometry.BetweenNodes(new Vec2(0, 0), to, size.X, size.Y, fromSide, side);
                        ...
                        await Assert.That(End).IsEqualTo(expected);
                        var tangent = End - Control2;
